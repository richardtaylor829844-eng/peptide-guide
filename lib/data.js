// ============================================================
// Backend / partner endpoints
// ============================================================
export const SITE_URL = "https://peptidereferenceguide.com";
export const SITE_NAME = "Peptide Reference Guide";

export const CHAT_ENDPOINT = "https://peptide-proxy.richardtaylor829844.workers.dev";
export const EMAIL_ENDPOINT = "https://formspree.io/f/mrerlwak";

// Big ad
export const AD_LINK = "https://uprisecg.com/";
export const AD_BRAND = "Uprise Consulting Group";
export const AD_LOGO = "https://uprisecg.com/wp-content/uploads/2025/03/Uprise-Consulting-Group-Inc-Logo-White.webp";
// Compact ad
export const COMPACT_AD_LINK = "https://vuoriclothing.com/";
export const COMPACT_AD_BRAND = "Vuori";

export const DISCLAIMER_VERSION = "2026-04-20";
...
// ----- Slug maps (URL-friendly) -----
export const PEPTIDE_SLUGS = {
  bpc157: "bpc-157",
  tb500: "tb-500",
  ll37: "ll-37",
  kpv: "kpv",
  ...
  nad: "nad",
  pinealon: "pinealon",
  cjcnodac: "cjc-1295-no-dac",
  cjcdac: "cjc-1295-with-dac",
...
export const STACK_SLUGS = {
  ...
  glow: "glow-blend",
  klow: "klow-blend",
  cjcipa: "cjc-1295-ipamorelin-blend",
  tesamipa: "tesamorelin-ipamorelin-blend"
};

export const CONCERN_SLUGS = {
  recovery: "injury-and-recovery",
  gut: "gut-health",
  weight: "weight-loss",
  aging: "anti-aging",
  ...
  skin: "skin-and-hair",
  sleep: "sleep-and-growth-hormone",
  sexual: "sexual-health"
};

// Reverse maps for lookup by slug
export const PEPTIDE_KEY_BY_SLUG = Object.fromEntries(
  Object.entries(PEPTIDE_SLUGS).map(([k, v]) => [v, k])
);
export const STACK_KEY_BY_SLUG = Object.fromEntries(
  Object.entries(STACK_SLUGS).map(([k, v]) => [v, k])
);
export const CONCERN_KEY_BY_SLUG = Object.fromEntries(
  Object.entries(CONCERN_SLUGS).map(([k, v]) => [v, k])
);
...
// Stack keys (subset of PEPS with cat==="Blends")
export const STACK_KEYS = ["wolverine","glow","klow","cjcipa","tesamipa"];

// Non-blend peptide keys
export const PEPTIDE_KEYS = Object.keys(PEPS).filter(k => !STACK_KEYS.includes(k));

export function peptideBySlug(slug) {
  const key = PEPTIDE_KEY_BY_SLUG[slug];
  return key ? { key, slug, ...PEPS[key] } : null;
}
export function stackBySlug(slug) {
  const key = STACK_KEY_BY_SLUG[slug];
  return key ? { key, slug, ...PEPS[key] } : null;
}
export function concernBySlug(slug) {
  const key = CONCERN_KEY_BY_SLUG[slug];
  if (!key) return null;
  const concern = CONCERNS.find(c => c.id === key);
  return concern ? { ...concern, slug } : null;
}
export function peptideUrl(key) {
  if (STACK_KEYS.includes(key)) return `/stacks/${STACK_SLUGS[key]}`;
  return `/peptides/${PEPTIDE_SLUGS[key]}`;
}
export function concernUrl(key) {
  return `/concerns/${CONCERN_SLUGS[key]}`;
}
